...
import { generate_semigroup, get_all_idempotent_elements, get_definite_k, get_highest_idempotent_power, is_abelian } from "../tsl/math/semigroup.js";
export function test_definite_k() {
    ...
    let idempotent_elements = get_all_idempotent_elements(strs, concat_with_suffix, (a, b) => a == b);
    if (idempotent_elements.length != Math.pow(alphabet.length, k)) {
        console.log("failed at idempotent_elements size");
        return false;
    }
    if (get_definite_k(strs, concat_with_suffix, (a, b) => a == b) != k) {
        console.log("failed at definite k");
        return false;
    }
    if (is_abelian(strs, concat_with_suffix, (a, b) => a == b)) {

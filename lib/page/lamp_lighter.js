        ...
        for (let i = 0; i < this.map_size; i++) {
            for (let j = 0; j < this.map_size; j++) {
                if (this.lamp_status[i][j] === 1) {
                    ...
                    const rowDist = Math.min(Math.abs(i - playerRow), this.map_size - Math.abs(i - playerRow));
                    const colDist = Math.min(Math.abs(j - playerCol), this.map_size - Math.abs(j - playerCol));
...
function render_description(size) {
    const n = (size !== null && size !== void 0 ? size : game.map_size).toString();
    const html = `
    ...
    for (let vis_i = 0; vis_i < game.map_size + 2; vis_i++) {
        for (let vis_j = 0; vis_j < game.map_size + 2; vis_j++) {
            ...
            if (actual_i === game.target_location[0] && actual_j === game.target_location[1]) {
                ...
                context.lineWidth = 4;
                context.strokeRect(vis_j * cell_width + 1, vis_i * cell_width + 1, cell_width - 2, cell_width - 2);
            }
        }
    }
    ...
    for (let vis_i = 0; vis_i < game.map_size + 2; vis_i++) {
        for (let vis_j = 0; vis_j < game.map_size + 2; vis_j++) {
            ...
            if (actual_i === game.current_location[0] && actual_j === game.current_location[1]) {
                ...
                context.lineWidth = 3;
                context.strokeRect(vis_j * cell_width + 2, // Offset by 2 pixels to avoid exact overlap with cell border
                vis_i * cell_width + 2, cell_width - 4, cell_width - 4);
            }
        }
    }

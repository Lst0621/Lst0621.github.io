...
function draw_lamp_lighter_canvas() {
    ...
    for (let vis_i = 0; vis_i < game.map_size + 2; vis_i++) {
        for (let vis_j = 0; vis_j < game.map_size + 2; vis_j++) {
            ...
            if (actual_i < 0)
                actual_i = game.map_size - 1; // Top border maps to bottom row
            if (actual_i >= game.map_size)
                actual_i = 0; // Bottom border maps to top row
            if (actual_j < 0)
                actual_j = game.map_size - 1; // Left border maps to rightmost column
            if (actual_j >= game.map_size)
                actual_j = 0; // Right border maps to leftmost column
    ...
    context.lineWidth = 3;
    context.strokeRect((game.target_location[1] + 1) * cell_width, (game.target_location[0] + 1) * cell_width, cell_width, cell_width);

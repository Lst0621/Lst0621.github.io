import { cat_subseq_of_blocklist, concat_same_string_lists_leq_n_times, get_all_prefixes, get_all_subseq_for_blocks, get_alphabet_from_strings, get_regex_for_disallowed_sub_seq, sub_empty_with_ep, subseq_remove_short } from "../tsl/lang/string.js";
...
function update() {
    ...
    for (let i = 0; i < idx.length; i++) {
        ...
        let s1 = pre1[a];
        let s2 = pre2[b];
        draw_state(ctx, [s1, s2].map(sub_empty_with_ep).join(","), [scale + a * scale, scale + b * scale], radius, a + b == 0 ? "lightblue" : visited[a][b] == 0 ? "lightgrey" :
    ...
    draw_table(document.getElementById("multiplication_table_1"), all_states, alphabet, (row, col) => {
        return get_next_state(all_states[row], alphabet[col], all_pres);
    }, a => a.map(sub_empty_with_ep).toString(), sub_empty_with_ep, a => a.map(sub_empty_with_ep).toString(), always("lightgreen"), always("lightblue"), (row, col) => {
    ...
    draw_table(document.getElementById("multiplication_table_2"), all_block_seq, alphabet, (row, col) => {
        return cat_subseq_of_blocklist(all_block_seq[row], ["", alphabet[col]], len, subs);
    }, a => subseq_remove_short(a).toString(), sub_empty_with_ep, a => subseq_remove_short(a).toString(), always("lightgreen"), always("lightblue"), (row, col) => {

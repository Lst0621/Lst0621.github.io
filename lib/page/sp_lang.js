import { get_all_prefixes } from "../tsl/lang/string.js";
...
function draw(s1, s2) {
    ...
    let l1 = pre1.length;
    let l2 = pre2.length;
    let idx1 = range(0, pre1.length);
    let idx2 = range(0, pre2.length);
    let idx = cartesian_product([idx1, idx2]);
    let scale = 120;
    let radius = 50;
    let arrows = [];
    let visited = create_2d_array(pre1.length, pre2.length, 0);
    ...
    for (let i = 0; i < idx.length; i++) {
        ...
        let s1 = pre1[a];
        let s2 = pre2[b];
        draw_state(ctx, "(" + s1 + "," + s2 + ")", [scale + a * scale, scale + b * scale], radius, visited[a][b] == 0 ? "lightgrey" : (a == l1 - 1 || b == l2 - 1 ? "orange" : "lightgreen"));
    }

// tsl/wasm/ts/wasm_api_linear_recur.ts
import wasmLinearRecur from "../tsl/wasm/wasm_out_v1/wasm_linear_recur.js";
...
function wasmMatrixPower(data, n, exponent) {
  ...
  const size = n * n;
  if (data.length !== size) {
    throw new Error(`Expected ${size} elements for ${n}\xD7${n} matrix.`);
  }
  const inBase = 2048;

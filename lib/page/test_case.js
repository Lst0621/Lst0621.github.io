...
function extractSuiteName(fullPath) {
...
function flattenResults(results) {
  ...
  for (const fileResult of results.testResults) {
    ...
    for (const assertion of fileResult.assertionResults) {
      index++;
      const name = assertion.ancestorTitles.length > 0 ? assertion.ancestorTitles.join(" > ") + " > " + assertion.title : assertion.title;
      rows.push({
        index,
        suite,
        name,
...
function renderTable(table, rows, results) {
  ...
  spacer.textContent = results.numPassedTests + "/" + results.numTotalTests + " passed, " + results.numFailedTests + " failed";
}
async function update_table() {
  const table = document.getElementById("test_case_table");
  try {

...
function update_poly() {
    ...
    for (let prime of gen_prime()) {
        ...
        console.log(vars);
        if (vars.every(v => v != 0)) {
            draw_table(table, [(poly_to_html(poly)),
                ("(" + poly_to_html(poly) + ") mod " + prime.toString())], range(0, prime), (row, col) => {
            ...
            }, i => i, c => c.toString(), e => e.toString(), always("lightgreen"), always("lightyellow"), always("lightblue"));
            break;
        }
        if (prime >= 97) {
            clear_table(table);

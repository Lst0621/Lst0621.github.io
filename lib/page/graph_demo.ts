...
void wasmResolveCache;
void cachedResolveKey;
void wasmResolveCacheGraphVersion;
...
let metricWorkers: Array<Worker | null> = [null, null, null];
...
function startMetricWorker(
    ...
    for (let i = 0; i < 3; i++) {
        if (metricWorkers[i]) {
            try { metricWorkers[i]?.terminate(); } catch {}
...
function renderAll(): void {
    ...
    if (!graphIoIsEditing) {
        const adj01ForIo: number[] = [];
        for (let i = 0; i < n; i++) {
            for (let j = 0; j < n; j++) {
                adj01ForIo.push(adj[i][j] ? 1 : 0);
            }
        }
        graphIoText = JSON.stringify(edgeListFromAdj01Based(adj01ForIo, n));

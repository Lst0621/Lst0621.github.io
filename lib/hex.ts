function drawHexagon(ctx: CanvasRenderingContext2D, cx: number, cy: number, hex_radius:number, color: string) {
    ctx.beginPath();
    for (let i = 0; i < 6; i++) {
...
export function hex_dist(a: number[], b: number[]): number {
    const x1 = a[0];
    const y1 = a[1];
    const x2 = b[0];
    const y2 = b[1];
...
export function drawHexGrid(ctx: CanvasRenderingContext2D, rows: number, cols: number, hex_radius: number,
                            get_color: { (col: number, row: number): string }) {
